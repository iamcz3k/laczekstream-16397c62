...
import { Bell, BellRing, Calendar, Clock, MapPin, Play, Radio, X } from "lucide-react";
...
export function FootballTab() {
  ...
  useEffect(() => {
    ...
    const loader =
      mode === "schedule"
        ? footballMatches().then(setEvents)
        : footballStreamMatches().then(setStreams);
    loader
      .catch(() => (mode === "schedule" ? setEvents([]) : setStreams([])))
    ...
    <div className="space-y-4">
      {userName && (
        <p className="text-sm text-muted-foreground">
          ⚽ Welcome back, <span className="font-bold text-foreground">{userName}</span> — here's
          today's action.
        </p>
      )}
        ...
        <div className="space-y-3">
          ...
          {events.map((ev) => {
            ...
            return (
              <article
                key={ev.id}
                className="glass-card rounded-[22px] p-4 transition-all duration-300 hover:border-primary/50"
              >
                <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <span
                    className={`inline-flex w-fit items-center gap-1 rounded-full px-2 py-1 text-xs font-bold ${isLive ? "bg-primary text-primary-foreground" : "glass text-muted-foreground"}`}
                  >
                    {isLive ? (
                ...
                <div className="space-y-3 sm:grid sm:grid-cols-[1fr_auto_1fr] sm:items-center sm:gap-4 sm:space-y-0">
                  <div className="flex items-center gap-3 sm:justify-end sm:text-right">
                    ...
                    <span className="min-w-0 text-base font-semibold leading-tight break-words">
                      {teamName(home)}
                    </span>
                  </div>
                  <div className="mx-auto flex w-fit min-w-24 items-center justify-center rounded-lg bg-secondary px-4 py-2 text-center">
                    {state === "pre" ? (
                      <span className="text-sm font-bold text-primary">{kickoff.time}</span>
                    ) : (
                ...
                <div className="mt-4 flex flex-wrap items-center justify-center gap-3 text-[11px] text-muted-foreground">
                  ...
                  {state === "pre" && ev.date && new Date(ev.date).getTime() > Date.now() && (
                    <NotifyButton
                      id={`sched-${ev.id}`}
                      title={`${teamName(home)} vs ${teamName(away)}`}
                      when={new Date(ev.date).getTime()}
                    />
                  )}
                </div>
              </article>
          ...
          {events.length === 0 && (
            <p className="py-20 text-center text-muted-foreground">No matches today.</p>
          )}
        </div>
      ) : (
        <div className="space-y-5">
          ...
          <div className="relative">
            <SearchInput
              ...
              onChange={(v) => {
                setSearch(v);
                if (v.trim().length > 2) trackSearch(v);
              }}
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {matches.map((match) => {
                  ...
                  return (
                    <div
                      key={match.id}
                      className="glass-card relative overflow-hidden rounded-[22px] text-left transition-all duration-300 hover:border-primary/50"
                    >
                      <Link
                        to="/football-stream/$matchId"
                      ...
                      >
                        ...
                        <div className="space-y-3 p-4 pb-2">
                          ...
                          <div className="flex items-center justify-between gap-3 text-sm text-muted-foreground">
                            ...
                            <span>
                              {match.viewers ? `${match.viewers.toLocaleString()} views` : "Stream"}
                            </span>
                          </div>
          ...
          {streams.length === 0 && (
            <p className="py-20 text-center text-muted-foreground">
              No football streams are live right now.
            </p>
          )}
        </div>
      )}
...
function NotifyButton({ id, title, when }: { id: string; title: string; when: number }) {
  ...
  async function notify() {
    ...
    if (ok) setScheduled(true);
      ...
      alert(
        "Notifications were blocked. Enable notifications in your browser settings to get a kickoff alert.",
      );
  }
