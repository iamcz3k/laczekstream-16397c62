...
import { Play, Star, SlidersHorizontal, X, User } from "lucide-react";
import { LoadingSpinner, EmptyState, SearchInput, PillToggle } from "@/components/shared";
import {
  tmdbTrending,
  tmdbSearch,
  tmdbPopular,
...
export function MoviesTab({ kind }: { kind: "movie" | "tv" }) {
  ...
  const [sortBy, setSortBy] = useState<
    "popularity.desc" | "vote_average.desc" | "release_date.desc" | "revenue.desc"
  >("popularity.desc");
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-3">
        <div ref={searchRef} className="relative flex-1 max-w-xl">
          <SearchInput
            ...
            onSubmit={search}
            placeholder={
              kind === "movie" ? "Search movies, actors, directors…" : "Search TV shows, actors…"
            }
          />
          {showSugg &&
          q.trim() &&
          (suggestions.movies.length || suggestions.tv.length || suggestions.people.length) ? (
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {it.year} {it.rating ? `· ★ ${it.rating.toFixed(1)}` : ""}
                            </p>
                          </div>
                        </Link>
                      ))}
        ...
        >
          ...
          {filtersActive
            ? ` · ${pickedGenres.length + (year ? 1 : 0) + (minRating ? 1 : 0) + (personFilter ? 1 : 0) + (sortBy !== "popularity.desc" ? 1 : 0)}`
            : ""}
        </button>
        <PillToggle
          options={[
            ...
            <div className="flex flex-wrap gap-2">
              {genres.map((g) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium transition ${pickedGenres.includes(g.id) ? "bg-primary text-primary-foreground" : "bg-secondary"}`}
                >
                  {g.name}
                </button>
              ))}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          ...
          {items.length === 0 && <EmptyState message="Nothing found." />}
        </div>
      )}
    </div>
  );
