...
export function CctvTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {playing && (/m3u8|mpd|playlist|manifest/i.test(playing.url) || playing.isStreaming ? (
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-background/95 backdrop-blur-2xl animate-in fade-in duration-200">
      <div className="flex items-center justify-between gap-3 border-b border-border glass px-4 py-4 sm:px-6">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-muted-foreground">{camera.city || camera.country || "Live CCTV"}</p>
        </div>
      ...
      <div className="flex flex-1 items-center justify-center bg-black">
        {camera.isIframe ? <iframe src={camera.url} title={camera.name} className="h-full w-full border-0" allow="autoplay; fullscreen; picture-in-picture" allowFullScreen /> : <img src={camera.url} alt={camera.name} className="max-h-full max-w-full object-contain" />}
      </div>
    </div>
  );
}